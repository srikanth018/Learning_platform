...
def extract_text(args):
	...
	for pageNum in range(len(pdfReader.pages)):
		...
		mytext += pageObj.extract_text()  # Updated to extract_text()
...
def main():
	...
	parser.add_argument("-v", "--voice", type=str, nargs=1,
					 ...
					 female voice preceded by -v. Default voice is male.")

	args = parser.parse_args()  # parsing all arguments in args var

	rate = args.rate[0] if args.rate is not None else 150  # default rate of speech
	voice = args.voice[0] if args.voice is not None else 'm'  # default voice is male
